# 本地安装

bws 支持从本地目录或文件安装浏览器版本，适用于已有浏览器安装包或绿色版的场景。本章详细介绍本地安装的各种方式和规则。

## 从目录安装

使用 `install -d` 命令可以从指定目录安装浏览器版本，自动识别目录下的浏览器文件。

### 基本用法

```bash
# 从目录自动识别并安装
bws i -d /path/to/browser-dir

# 指定版本号安装（当无法自动识别时）
bws i -d /path/to/browser-dir chrome@120
```

### 强制重新安装

默认情况下，已安装的版本会被跳过。使用 `-f` 参数可以强制重新安装：

```bash
bws i -d /path/to/browser-dir -f
```

### 安装过程

安装过程中会实时显示进度，包括：

- 当前正在处理的文件
- 识别到的浏览器名称和版本
- 安装进度百分比
- 成功/失败状态

无法识别的文件会即时提示，但不会中断整体安装流程。

### 示例输出

```
扫描目录: D:\browsers
发现 5 个文件，开始识别...

✓ Chrome_120.0.6099.109_Windows_x64.exe → chrome 120.0.6099.109
✓ firefox-121.0-win64.zip → firefox 121.0
✗ unknown_setup.exe → 无法识别
✓ chrome-79.0.3945.79.zip → chrome 79.0.3945.79
✓ chrome-79.0.3945.79.tar.gz → chrome 79.0.3945.79

安装完成：成功 4 个，失败 1 个
```

### 适用场景

- 绿色版浏览器目录
- 解压后的浏览器安装目录
- 手动构建的浏览器目录

## 从文件安装

使用 `install --from-file` 命令可以从单个文件安装，支持压缩包和安装包格式。

### 基本用法

```bash
# 从压缩包安装（自动识别）
bws i --from-file /path/to/chrome-setup.exe
bws i --from-file /path/to/chrome.zip

# 指定版本号安装（当文件名无法识别时）
bws i --from-file /path/to/file.exe chrome@120
```

### 两种安装方式的对比

| 特性 | `install -d` | `install --from-file` |
|------|--------------|----------------------|
| 处理对象 | 目录 | 单个文件 |
| 自动识别 | 是（扫描目录下所有文件） | 是（根据文件名） |
| 手动指定版本 | 支持 | 支持 |
| 适用场景 | 包含多个浏览器文件的目录 | 单个压缩包或安装包 |

## 支持的文件格式

bws 支持 zip、7z、tar.gz、tar.bz2、tar.xz、tar.zst、gz、bz2、xz、zst、exe、dmg、cab、apk、jar、war 等多种文件格式，主要包括以下类别：

### 压缩包格式

| 格式 | 扩展名 | 说明 |
|------|--------|------|
| ZIP | `.zip` | 最常见的压缩格式 |
| 7-Zip | `.7z` | 高压缩比格式 |
| Tar | `.tar` | Unix 归档格式 |
| Tar+Gzip | `.tar.gz`, `.tgz` | 常见 Unix 压缩格式 |
| Tar+Bzip2 | `.tar.bz2`, `.tbz2` | 高压缩比格式 |
| Tar+XZ | `.tar.xz`, `.txz` | 高压缩比格式 |
| Tar+Zstd | `.tar.zst` | zstd 压缩格式 |
| Gzip | `.gz` | Gzip 单文件压缩 |
| Bzip2 | `.bz2` | Bzip2 单文件压缩 |
| XZ | `.xz` | XZ 单文件压缩 |
| Zstd | `.zst` | Zstd 单文件压缩 |

### 可执行文件与特殊格式

| 格式 | 扩展名 | 说明 |
|------|--------|------|
| Windows 可执行 | `.exe` | 自解压安装包（作为 zip 解压），不执行安装 |
| macOS 磁盘镜像 | `.dmg` | macOS 磁盘镜像（zip-based） |
| Windows Cabinet | `.cab` | Windows Cabinet 压缩文件 |

### 其他归档格式

| 格式 | 扩展名 | 说明 |
|------|--------|------|
| Android APK | `.apk` | Android 安装包（zip-based） |
| Java JAR | `.jar` | Java 归档文件（zip-based） |
| Web WAR | `.war` | Web 应用归档（zip-based） |

> **魔数检测**：除文件扩展名外，bws 还会通过文件头魔数（Magic Byte）识别归档格式。例如，ZIP 文件的 PK 签名、7z 文件的 `7z` 签名等。因此，即使文件缺少正确的扩展名，bws 仍能根据文件头特征正确识别和处理。

### 目录格式

直接包含浏览器可执行文件的目录也可以被识别和安装。

## 文件名自动识别规则

bws 通过文件名或目录名中的关键词自动识别浏览器信息，包括浏览器名称、版本号、平台、架构和渠道。

### 识别要素

| 要素 | 识别关键词 | 示例 |
|------|-----------|------|
| **浏览器名** | chrome, chromium, firefox | `chrome`, `firefox` |
| **版本号** | 形如 `x.y.z.w` 的数字组合 | `120.0.6099.109` |
| **架构** | `win64`/`x64`/`amd64` 或 `win32`/`x86`/`386` | `x64`, `win64` |
| **平台** | `windows`/`win`、`macos`/`mac`、`linux` | `windows`, `win` |
| **渠道** | `stable`, `beta`, `dev`, `canary`, `esr` | `stable`, `beta` |

### 文件名示例

以下是一些可以被正确识别的文件名示例：

```
44.0.2403.107_chrome64_stable_windows_installer.exe
GoogleChrome_148.0.7778.167_Windows_x64_Offline.exe
firefox-115.0esr-win64.zip
Chrome_120.0.6099.109_Windows_x64.zip
chromium-85.0.4183.121-linux-x64.tar.gz
firefox-121.0-x64.tar.bz2
```

### 识别优先级

1. 首先识别浏览器名称
2. 然后提取版本号
3. 接着识别架构、平台、渠道
4. 如果缺少部分信息，使用默认值（如默认 stable 渠道）

## 无法识别时的处理方法

如果文件名或目录名无法被自动识别，可以通过手动指定版本的方式进行安装。

### 指定版本安装

使用 `install -d` 或 `install --from-file` 命令时，在末尾加上版本标识：

```bash
# 从目录安装，手动指定版本
bws i -d /path/to/dir chrome@120

# 从文件安装，手动指定版本
bws i --from-file /path/to/file.exe chrome@120
```

### 指定版本的格式

版本标识的格式为 `browser@version`：

- `browser`：浏览器名称（如 chrome、firefox、chromium）或短别名（gc、ff、cm）
- `version`：版本号，可以是完整版本号或部分版本号

示例：

```bash
bws i --from-file ./mystery.exe gc@120.0.6099.109
bws i -d ./my-browser ff@115.0esr
```

### 指定渠道和架构

如果需要更精确的指定，还可以配合其他参数：

```bash
bws i --from-file ./file.exe chrome@120 --channel beta
```

### 无法识别的常见原因

1. 文件名中没有明显的浏览器名称关键词
2. 版本号格式不标准
3. 缺少架构或平台标识（会使用系统默认值）
4. 自定义命名的文件无法匹配规则

当遇到无法识别的文件时，建议使用手动指定版本的方式进行安装。
