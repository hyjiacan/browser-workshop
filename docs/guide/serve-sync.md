...
如果同步正在进行中，请求会被静默忽略（no-op），仍返回成功响应。

如果同步未启用（未配置同步源），`/api/v1/sync/status` 仍返回 200，`data` 中 `progress` 字段提示同步未启用；`/api/v1/sync/trigger` 返回 503。
...
3. **同步冲突**：手动触发同步时，如果定时同步正在进行，会被静默忽略
