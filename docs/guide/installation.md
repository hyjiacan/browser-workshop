...
将 `bws.exe` 放在任意目录，首次运行后自动在同目录生成 `bws-data/` 文件夹，数据（版本、缓存、日志、运行时等）存储在其中；配置文件（`bws-client.ini` 和 `bws-serve.ini`）则位于与 `bws.exe` 同目录下。整个程序可以连同数据一起拷贝到 U 盘或其他电脑上使用。
...
设置后，所有数据（版本、缓存、日志、运行时等）将存储到指定目录中。配置文件（`bws-client.ini` 和 `bws-serve.ini`）始终位于与 bws 可执行文件同目录下。清空该配置可恢复默认的便携模式：
