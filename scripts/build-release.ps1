...
.DESCRIPTION
    ...
    Zip file names include platform and architecture identifiers,
    while the binary file name itself does not.

    Output example:
      dist/bws_windows_amd64.zip  (contains bws.exe)
      dist/bws_linux_amd64.zip    (contains bws)
      dist/bws_darwin_arm64.zip   (contains bws)
...
.PARAMETER Targets
    ...
    Defaults to: windows/amd64, windows/386, linux/amd64, linux/arm64, darwin/amd64, darwin/arm64
...
.EXAMPLE
    .\scripts\build-release.ps1 -Targets @("windows/amd64", "linux/amd64")
...
# Locate project root (script is in scripts/ subdirectory)
...
foreach ($Target in $Targets) {
    ...
    $ZipName = "bws_${GOOS}_${GOARCH}.zip"
